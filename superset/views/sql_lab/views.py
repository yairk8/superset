...
class TabStateView(BaseSupersetView):
    ...
    def post(self) -> FlaskResponse:
        ...
        except (SQLAlchemyError, KeyError, JSONDecodeError):
            logger.exception("Failed to create tab state")
            db.session.rollback()
            return json_error_response(
                "An error occurred while creating the tab state", 400
            )
    ...
    def delete(self, tab_state_id: int) -> FlaskResponse:
        try:
            ...
            logger.exception("Failed to delete tab state %s", tab_state_id)
            db.session.rollback()
            return json_error_response(
                "An error occurred while deleting the tab state", 400
            )
    ...
    def activate(self, tab_state_id: int) -> FlaskResponse:
        try:
            ...
            logger.exception("Failed to activate tab state %s", tab_state_id)
            db.session.rollback()
            return json_error_response(
                "An error occurred while activating the tab state", 400
            )
    ...
    def put(self, tab_state_id: int) -> FlaskResponse:
        ...
        try:
            ...
            logger.exception("Failed to update tab state %s", tab_state_id)
            db.session.rollback()
            return json_error_response(
                "An error occurred while updating the tab state", 400
            )
    ...
    def migrate_query(self, tab_state_id: int) -> FlaskResponse:
        try:
            ...
            logger.exception("Failed to migrate query for tab state %s", tab_state_id)
            db.session.rollback()
            return json_error_response(
                "An error occurred while migrating the query", 400
            )
    ...
    def delete_query(self, tab_state_id: int, client_id: str) -> FlaskResponse:
        try:
            ...
            logger.exception(
                "Failed to delete query %s from tab state %s",
                client_id,
                tab_state_id,
            )
            db.session.rollback()
            return json_error_response(
                "An error occurred while deleting the query", 400
            )
...
class TableSchemaView(BaseSupersetView):
    ...
    def post(self) -> FlaskResponse:
        try:
            ...
            db.session.rollback()
            return json_error_response(
                "An error occurred while creating the table schema", 400
            )
    ...
    def delete(self, table_schema_id: int) -> FlaskResponse:
        try:
            ...
            logger.exception("Failed to delete table schema %s", table_schema_id)
            db.session.rollback()
            return json_error_response(
                "An error occurred while deleting the table schema", 400
            )
    ...
    def expanded(self, table_schema_id: int) -> FlaskResponse:
        try:
            ...
            logger.exception(
                "Failed to update expanded state for table schema %s",
                table_schema_id,
            )
            db.session.rollback()
            return json_error_response(
                "An error occurred while updating the table schema", 400
            )
